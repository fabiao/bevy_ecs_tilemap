...
use bevy::{
    ...
    prelude::{Component, Entity, GlobalTransform, InheritedVisibility, Mesh, Vec3},
    render::{
...
impl RenderChunk2d {
    ...
    pub fn new(
        ...
        let transform_affine = transform.compute_affine();
    ...
    pub fn get_transform_affine(&self) -> Affine3A {
        self.transform_affine
    }

    pub fn intersects_frustum(&self, frustum: &ExtractedFrustum) -> bool {
        frustum.intersects_obb(&self.aabb, &self.transform_affine)
    }
    ...
    pub fn update_geometry(
        ...
        if dirty_local_transform || dirty_global_transform {
            ...
            self.transform_affine = self.transform.compute_affine();
        }
    }
